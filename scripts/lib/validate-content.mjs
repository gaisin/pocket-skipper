...
// Слово о стороне: начинается с корня «лев»/«прав» (в том числе влево, справа, налево) или это «часовой (стрелки)»;
// «часов» (часы) не считается. Корень ищется только в начале слова, поэтому «рулевой», «управление»,
// «поправить», «подправить», «исправить», «отправить» не считаются.
const SIDE_WORD = /^(?:в|с|на)?(?:лев|прав)|^часов[а-яё]/;
// Слова с тем же началом, но не о стороне: правило, правильно, править, правда, правка, правление, правовой;
// вправе («имеет право»); справка, справочник, справиться, справедливо; направление, направить;
// «прав», «правы», «права» (правота, права на управление).
const NOT_SIDE_WORD = /^(?:прави[лт]|правд|правк|правл|правов|вправе$|справ(?:к|оч|и|л|я|ед)|направ(?:л|и|ь|я)|прав(?:а|ы|ам|ами|ах)?$)/;

function sideWords(text) {
  return (text.toLowerCase().match(/[а-яё]+/g) ?? []).filter((w) => SIDE_WORD.test(w) && !NOT_SIDE_WORD.test(w));
}
...
function checkSidePairs(r, err) {
  for (const t of maneuverTexts(r)) {
    ...
    else if (r.mirror === true) {
      // В зеркальном манёвре сторона вне пары осталась бы той же и в отражённом варианте.
      for (const w of sideWords(rest)) err(`слово о стороне «${w}» вне пары [[..|..]] в «${t}»`);
    }
  }
  if (r.mirror === true) {
