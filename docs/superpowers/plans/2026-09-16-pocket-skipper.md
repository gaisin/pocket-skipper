...
> **Контроллер:** перед этой задачей спросить у пользователя разрешение на `npm install -D @playwright/test@1.63.0` (ставится в `node_modules/` проекта и удаляется вместе с этой папкой; Chromium для Playwright уже скачан на компьютер).
...
Группы:
- «Документы» - паспорт, сертификат IYT Bareboat Skipper, сертификат УКВ-оператора (уточнить у пользователя, есть ли он, и что требует чартерная компания), договор чартера, судовая роль (crew list), страховка; требования Турции к шкиперу и экипажу чартерной яхты - только по `web`-источникам (сайт чартерной компании, официальные турецкие источники).
...
| `anchoring` | Якорь и швартовка | 6 | модуль 4 с. 83, модуль 14 с. 217 |
...
- Картинки:
  - огни - `image.kind = "lights"`; бортовые огни рисовать слева красный, справа зелёный, если судно идёт на наблюдателя; топовые белые - выше; в `label` - словесное описание;
